<template>
  <div class="mt-10">
    <h2 class="font-semibold text-2xl">Supervisor</h2>
    <div class="">
      <div class="sm:-mx-6 lg:-mx-8">
        <div class="py-4 inline-block min-w-full sm:px-6 lg:px-8">
          <div class="">
            <div class="flow-root">
              <router-link :to="{ name: 'addSupervisor' }" tag="button"
            ...
            <table class="table-fixed">
              ...
              <tbody class="border">
                <tr v-for="supervisor in supervisors" class="bg-white border-b" :key="supervisor.id"
                  :class="isChecked(supervisor.id) ? 'table-primary' : ''">
                  <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                    <input type="checkbox" :value="supervisor.id" v-model="checked" />
                  </td>
                  <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    {{ supervisor.name }}
                  </td>
                  <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    {{ supervisor.ic_number }}
                  </td>
                  <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    {{ supervisor.email }}
                  </td>
                  ...
                  <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    <router-link :to="{ name: 'editSupervisor', params: { id: supervisor.id } }" tag="button"
      ...
      <div v-if="onSave == 1"
        ...
        <div class="text-xl font-normal  max-w-full flex-initial">
          Supervisor has been <span v-if="add == 1">added</span><span v-else>updated</span> successfully.</div>
      ...
      <div v-if="onDelete == 1"
        ...
        role="alert">
        <div class="text-xl font-normal  max-w-full flex-initial">
          Supervisor(s) deleted successfully.</div>
      ...
      <div>
        <pagination :data="supervisors" @pagination-change-page="getSupervisors"></pagination>
      </div>
    </div>
  </div>
  ...
  <div v-show="alertModal" class="absolute inset-0 flex items-center justify-center bg-gray-700 bg-opacity-50">
    <div class="max-w-2xl p-6 bg-white rounded-md shadow-xl">
      <div class="flex items-center justify-between">
        <h3 class="text-2xl">Cannot Delete Supervisor(s)</h3>
      </div>
      <div class="mt-4">
        <p class="mb-4">
          One or more selected supervisor has related in work order.
        </p>
...
export default {
    ...
    selectPage: function (value) {
      ...
      if (value) {
        this.supervisors.forEach((supervisor) => {
          this.checked.push(supervisor.id);
        });
      } else {
        this.checked = [];
  ...
  methods: {
    ...
    isChecked(supervisor_id) {
      return this.checked.includes(supervisor_id);
    },
    deleteRecords() {
    ...
    confirmDelete() {
      ...
      axios
      .delete("/api/supervisor/multiDelete/" + this.checked)
      .then((response) => {
